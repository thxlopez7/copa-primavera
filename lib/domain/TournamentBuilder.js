...
export class TournamentBuilder {
  ...
  static buildGraph(matches) {
    ...
    // 1. Agrupar Fase de Zonas (Identificador: m.result?.group_name o m.round_name === 'Fase de Grupos')
    const groupMatches = matches.filter(m => m.round_name === 'Fase de Grupos' || (m.round && m.round.startsWith('GROUP_')));
    const zonesMap = {};
    
    groupMatches.forEach(m => {
      // Intentar sacar de result.group_name primero (nueva implementación)
      let zoneId = m.result?.group_name;
      
      // Fallback para datos legacy (si existen con 'GROUP_A_P1')
      if (!zoneId && m.round) {
    ...
    // 2. Agrupar Fase Eliminatoria (Identificador: m.round_name empieza con 'Cuartos', 'Semis', etc o legacy 'KO_')
    const koMatches = matches.filter(m => m.round_name !== 'Fase de Grupos' && (!m.round || !m.round.startsWith('GROUP_')));
    const koMap = {};

    koMatches.forEach(m => {
      // Legacy koMap logic based on m.round
      if (m.round && m.round.startsWith('KO_')) {
