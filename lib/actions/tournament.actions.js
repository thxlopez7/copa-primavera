...
export async function publishTournamentPhase(config) {
  try {
    ...
    if (format === 'groups' || format === 'both') {
      // Barajar (shuffle) las parejas aleatoriamente
      const shuffledPairs = [...pairs].sort(() => Math.random() - 0.5);
      
      // Cálculo de Zonas: apuntamos a ~3 o 4 parejas por zona.
      const groupsCount = Math.max(1, Math.ceil(shuffledPairs.length / 4));
      
      // Distribuir las parejas en los grupos equitativamente
 ...
 * cruzando los datos con los equipos para mostrar el Bracket.
 */
export async function fetchCategoryMatches(categoryId) {
  try {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        team1:team1_id(id, player1:player1_id(last_name), player2:player2_id(last_name)),
        team2:team2_id(id, player1:player1_id(last_name), player2:player2_id(last_name))
      `)
      .eq('category_id', categoryId)
      .order('id'); // En realidad el orden no importa tanto, el Bracket lo organiza por round_name
